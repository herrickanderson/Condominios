...
const props = defineProps({
  gastos: Array,       // [{ id_gasto, descripcion, monto_total, monto_asignado, distribuciones_count }, ...]
  tipos: Array,        // [{ id_tipo_gasto, nombre }, ...]
  edificios: Array,    // [{ id_edificio, nombre }, ...]
  unidades: {
    type: Array,
    default: () => []
  },
...
const form = useForm({
  ...
  id_tipo_gasto: '',
  id_edificio: '',
  distribution_scope: 'condominium', // 'condominium', 'tower', 'unit'
  target_tower: '',
  target_unit: '',
  monto_detalle: '',
  descripcion_detalle: '',
...
// Filtrar unidades por torre si distribution_scope === 'unit'
...
// Limpiar target_tower y target_unit al cambiar el scope
...
function submitForm() {
  if (form.id_edificio === '') form.id_edificio = null;
  if (form.target_tower === '') form.target_tower = null;
  if (form.target_unit === '') form.target_unit = null;

  form.post(route('detalle_gasto_comun.store'), {
    onSuccess: () => form.reset('file_url'),
  });
}
</script>

<template>
  <Head title="Crear Detalle Gasto Común" />
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between bg-green-600 p-4 rounded-md">
        <h3 class="text-xl font-semibold text-white">Crear Detalle Gasto Común</h3>
      ...
      <div class="mx-auto max-w-4xl bg-white shadow-lg rounded-xl p-8 border border-gray-200">
        ...
        <form class="space-y-6" @submit.prevent="submitForm">
          ...
          <div>
            ...
            <div class="relative">
              ...
              >
                ...
                <!-- Solo aparecerán los que no tienen distribuciones (según el controlador) -->
          ...
          <div v-if="selectedGasto" class="p-3 mt-1 border rounded bg-gray-50 text-sm">
            <p class="mb-1">
              <strong>Monto Total:</strong> {{ selectedGasto.monto_total ?? 'N/A' }}
            </p>
            <p class="mb-1">
              <strong>Asignado:</strong> {{ selectedGasto.monto_asignado ?? 0 }}
            </p>
            <p>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <select
                ...
                class="w-full border-gray-300 rounded-md pl-10"
                required
                :disabled="!form.id_gasto"
              >
                <option disabled value="">Seleccione un tipo</option>
                <option v-for="t in props.tipos" :key="t.id_tipo_gasto" :value="t.id_tipo_gasto">
                  {{ t.nombre }}
                </option>
              </select>
            </div>
          ...
          <!-- Monto Detalle -->
          <div>
            <InputLabel for="monto_detalle" value="Monto" />
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
