...
// Inicializamos el formulario usando la propiedad "servicios_extras" (snake_case)
const form = useForm({
  nombre: props.extencion.nombre,
  id_edificio: props.extencion.id_edificio,
  area: props.extencion.area,
  unidad_medida: props.extencion.unidad_medida,
  tipo_extencion: props.extencion.tipo_extencion,
  cobro_unico: props.extencion.cobro_unico ? String(props.extencion.cobro_unico) : '0',
  servicios_extra: (props.extencion.servicios_extras || []).map(se => ({
...
function addService() {
  ...
  if (isNaN(perc) || perc < 0) {
    errorMsg.value = 'Porcentaje inválido (debe ser ≥ 0)';
    return;
  }
...
function submit() {
  form.put(route('extenciones.update', props.extencion.id_extencion));
}
</script>

<template>
  <Head title="Editar Extención" />
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between bg-green-600 p-4 rounded-md">
        <h3 class="text-xl font-semibold text-white">Editar Extención</h3>
    ...
    <div class="py-12">
      <div class="mx-auto max-w-4xl bg-white shadow-lg rounded-xl p-8 border border-gray-200">
        <h3 class="text-2xl font-bold text-gray-700 mb-6 text-center">Actualizar Extención</h3>
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <!-- Campo de Cobro Único para Estacionamiento -->
          ...
          <!-- Sección para agregar y visualizar servicios extras -->
          <div>
          ...
          <div class="flex justify-center">
            <PrimaryButton class="flex items-center space-x-1">
              <PlusIcon class="h-5 w-5" />
              <span>Actualizar Extención</span>
            </PrimaryButton>
          </div>
        </form>
      </div>
